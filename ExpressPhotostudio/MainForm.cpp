...
/*	Bitmap^ stretchBackground(Bitmap^ background, Bitmap^ image)
{
System::Drawing::Imaging::BitmapData^ imageData = image->LockBits(Rectangle(0, 0, image->Width, image->Height), System::Drawing::Imaging::ImageLockMode::ReadWrite, System::Drawing::Imaging::PixelFormat::Format32bppArgb);
System::Drawing::Imaging::BitmapData^ backData = background->LockBits(Rectangle(0, 0, background->Width, background->Height), System::Drawing::Imaging::ImageLockMode::ReadWrite, System::Drawing::Imaging::PixelFormat::Format24bppRgb);
for (int y = 0; y < image->Height; y++)
{
IntPtr position = ((IntPtr)imageData->Scan0) + y * imageData->Stride;
Byte *imagePos = ((Byte *)position.ToPointer());
position = ((IntPtr)backData->Scan0) + y * backData->Stride;
Byte *backPos = ((Byte *)position.ToPointer());
int x = 0;
while (x < image->Width)
{
if (background->Width < image->Width)
{
for (int j = 0; j < image->Width / background->Width + 1; j++)
{
*imagePos = *backPos;
*(imagePos + 1) = *(backPos + 1);
*(imagePos + 2) = *(backPos + 2);
*(imagePos + 3) = 255;
imagePos += 4;
}
}
x += image->Width / background->Width + 1;
backPos += 3;
}
}
image->UnlockBits(imageData);
background->UnlockBits(backData);
return image;
}

Bitmap^ mergeBackground(Bitmap^ image, Bitmap^ background)
{
System::Drawing::Imaging::BitmapData^ imageData = image->LockBits(Rectangle(0, 0, image->Width, image->Height), System::Drawing::Imaging::ImageLockMode::ReadWrite, System::Drawing::Imaging::PixelFormat::Format32bppArgb);
System::Drawing::Imaging::BitmapData^ backData = background->LockBits(Rectangle(0, 0, background->Width, background->Height), System::Drawing::Imaging::ImageLockMode::ReadWrite, System::Drawing::Imaging::PixelFormat::Format24bppRgb);
for (int y = 0; y < image->Height; y++)
...
while (x < image->Width)
{
*imagePos = (*backPos * (WHITE - *(imagePos + 3)) + *imagePos * *(imagePos + 3)) / 255;
*(imagePos + 1) = (*(backPos + 1) * (WHITE - *(imagePos + 3)) + *(imagePos + 1) * *(imagePos + 3)) / 255;
*(imagePos + 2) = (*(backPos + 2) * (WHITE - *(imagePos + 3)) + *(imagePos + 2) * *(imagePos + 3)) / 255;
*(imagePos + 3) = 255;
imagePos += 4;
backPos += 3;
x++;
